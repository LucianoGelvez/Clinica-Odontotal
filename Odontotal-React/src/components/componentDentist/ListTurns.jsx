    ...
    <>
      {data.map((item) => (
        <tr key={item.id}>
          ...
          <td>
            <button onClick={() => seeDetails(item.pacienteId)} className="btn-delete">Historial Paciente</button>
          </td>
          <td>
            <Link to={`HistorialDelPaciente/${item.pacienteId}`}>Historial Paciente</Link>
          </td>
        </tr>
      ))}
    </>
