...
const infoValidacionCuenta = (responseData, setFormData) => {
  const errorToast = (error = null) => {
    Swal.fire({
      ...
      text: error
        ? error
        : "Ha ocurrido un error al reenviar el correo de validación.",
      icon: "error",
  ...
  const html = `
    <div style="text-align: left;">
      ...
      }</b>, recuerda validar tu cuenta.</p>
      <p>Se ha enviado un correo electrónico a <b>${
        responseData.email
      }</b> con un enlace para validar tu cuenta.</p>
  ...
  Swal.fire({
    ...
    preConfirm: async () => {
      try {
        ...
        const response = await fetch(
          baseUrl.url + `/mail/validacion/${responseData.id}`
        );
    ...
    .catch(async (err) => {
      await errorToast();
      return infoValidacionCuenta(response, setFormData, {
        retries,
        initialTime,
      });
    });
