...
const FormTurn =({ data, onCancelar, informacionCompleta, jwt  }) => {
  ...
  const [nombreOdontologo, setNombreOdontologo] = useState(data.nombreOdontologo);
  const [apellidoOdontologo, setApellidoOdontologo] = useState(data.apellidoOdontologo);
  ...
  const [apellidoPaciente, setApellidoPaciente] = useState(data.apellidoPaciente);
  const [documentoPaciente, setDocumentoPaciente] = useState(data.documentoPaciente);
  const [fecha, setFecha] = useState(data.fecha);
  const [hora, setHora] = useState(data.hora);
  const [motivo, setMotivo] = useState(data.motivo);
  const [trabajoRealizado, setTrabajoRealizado] = useState(data.trabajoRealizado);
    ...
    <section className='table-container'>
      ...
      <table>
        ...
        <div className='editable'>
            <label>Motivo de la visita del paciente:</label><textarea className='editable_motive'  style={{height: "40%"}} type="text" value={motivo} onChange={(e) => setMotivo(e.target.value)} />
            <label>Trabajo Realizado:</label><textarea className='editable_worktodone' type="text" value={trabajoRealizado} onChange={(e) => setTrabajoRealizado(e.target.value)} />
            <div>
